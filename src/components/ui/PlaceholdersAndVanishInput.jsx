...
export function PlaceholdersAndVanishInput({ placeholders, onChange, onSubmit }) {
    ...
    const startAnimation = () => {
        if (!placeholders || !Array.isArray(placeholders)) return; // Early return if placeholders is invalid
    ...
    const vanishAndSubmit = () => {
        ...
        if (value && inputRef.current && newDataRef.current.length > 0) {
    ...
    return (
        <form
        className={cn(
            "w-[80%] relative max-w-xl text-[#F5F5F5] bg-[#2D2D2D] placeholder-[#B0B0B0] h-12 rounded-lg overflow-hidden transition duration-200",
        ...
        <div className="absolute inset-0 flex items-center rounded-full pointer-events-none">
            ...
            {!value && placeholders && Array.isArray(placeholders) && (
                <motion.p
